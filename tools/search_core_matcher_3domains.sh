...
if [[ $# -lt 1 || $# -gt 2 ]]; then
  echo "Usage: bash tools/search_core_matcher_3domains.sh CHECKPOINT [OUTPUT_DIR]"
  exit 2
fi

CHECKPOINT=$1
OUTPUT_DIR=${2:-./core_matcher_3domain_search}
...
if summary:
    best = summary[0]
    print(
        "Best by official three-domain arithmetic mean: "
