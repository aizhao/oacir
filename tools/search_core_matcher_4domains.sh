#!/usr/bin/env bash
set -uo pipefail

if [[ $# -lt 1 || $# -gt 2 ]]; then
  echo "Usage: bash tools/search_core_matcher_4domains.sh CHECKPOINT [OUTPUT_DIR]"
  exit 2
fi

CHECKPOINT=$1
OUTPUT_DIR=${2:-./core_matcher_4domain_search}

if [[ ! -f "$CHECKPOINT" ]]; then
  echo "Checkpoint not found: $CHECKPOINT"
  exit 2
fi

mkdir -p "$OUTPUT_DIR/logs" "$OUTPUT_DIR/done"

DOMAINS=(Fashion Car Product Landmark)
FUSIONS=(0.10 0.20 0.30)
LAMBDAS=(0.00 0.25 0.50 0.75)

export PYTORCH_CUDA_ALLOC_CONF=${PYTORCH_CUDA_ALLOC_CONF:-max_split_size_mb:128}
export HF_HUB_OFFLINE=1
export TRANSFORMERS_OFFLINE=1
export HF_HUB_DISABLE_TELEMETRY=1
export TOKENIZERS_PARALLELISM=false

run_one() {
  local fusion=$1
  local lambda_id=$2
  local domain=$3
  local tag="fusion_${fusion}_lambda_${lambda_id}_${domain,,}"
  local log_path="$OUTPUT_DIR/logs/${tag}.log"
  local done_path="$OUTPUT_DIR/done/${tag}.done"

  if [[ -f "$done_path" ]]; then
    echo "[skip] fusion=$fusion lambda=$lambda_id domain=$domain"
    return 0
  fi

  echo "[run] fusion=$fusion lambda=$lambda_id domain=$domain"
  if python evaluate.py \
    --dataset "$domain" \
    --data-root ./Datasets/OACIRR \
    --blip-model-name oacir_latent \
    --blip-model-weight "$CHECKPOINT" \
    --vit-backbone pretrain \
    --transform targetpad \
    --target-ratio 1.25 \
    --use-latent-matching \
    --highlight-inference \
    --latent-matcher core_matcher \
    --core-matcher-temp 0.07 \
    --core-matcher-lambda-id "$lambda_id" \
    --core-matcher-fusion-weight "$fusion" \
    --core-matcher-topk 50 \
    --core-matcher-cycle-weight 0.25 \
    --core-matcher-text-weight 0.25 \
    --core-matcher-bbox-floor 0.05 \
    --core-matcher-query-chunk-size 2 \
    --region-topk 16 \
    --region-topk-max 96 \
    --region-area-scale 1.5 \
    --region-spatial-kernel 3 \
    --region-spatial-weight 0.15 \
    --region-temperature 0.07 \
    --latent-chunk-size 64 \
    --latent-gallery-chunk-size 1024 \
    --eval-batch-size 16 \
    --val-feature-batch-size 32 \
    --val-feature-cache './cache/oacirr_{variant_lower}_val_vitg_targetpad125_raw_fp16.pt' \
    --save-memory 2>&1 | tee "$log_path"; then
    touch "$done_path"
  else
    echo "[failed] fusion=$fusion lambda=$lambda_id domain=$domain"
    return 1
  fi
}

# Pure AdaFocal baseline. lambda_id has no effect when fusion is zero.
for domain in "${DOMAINS[@]}"; do
  run_one 0.00 0.00 "$domain"
done

for fusion in "${FUSIONS[@]}"; do
  for lambda_id in "${LAMBDAS[@]}"; do
    for domain in "${DOMAINS[@]}"; do
      run_one "$fusion" "$lambda_id" "$domain"
    done
  done
done

python - "$OUTPUT_DIR" <<'PY'
import csv
import json
import re
import sys
from collections import defaultdict
from pathlib import Path

output_dir = Path(sys.argv[1])
pattern = re.compile(
    r"fusion_(?P<fusion>[0-9.]+)_lambda_(?P<lambda>[0-9.]+)_"
    r"(?P<domain>fashion|car|product|landmark)\.log$"
)
rows = []

for log_path in sorted((output_dir / "logs").glob("*.log")):
    match = pattern.match(log_path.name)
    if not match:
        continue
    text = log_path.read_text(errors="replace")
    objects = []
    for candidate in re.findall(r"\{[^{}]+\}", text, flags=re.DOTALL):
        try:
            value = json.loads(candidate)
        except json.JSONDecodeError:
            continue
        if "R@1" in value and "R_ID@1" in value:
            objects.append(value)
    if not objects:
        print(f"Warning: no result JSON found in {log_path}", file=sys.stderr)
        continue
    metrics = objects[-1]
    rows.append({
        "fusion": float(match.group("fusion")),
        "lambda_id": float(match.group("lambda")),
        "domain": match.group("domain").title(),
        **{key: metrics[key] for key in (
            "R_ID@1", "R_ID@3", "R_ID@5", "R@1", "R@5", "R@10", "R@50"
        )},
    })

metric_names = ["R_ID@1", "R_ID@3", "R_ID@5", "R@1", "R@5", "R@10", "R@50"]
all_path = output_dir / "all_results.csv"
with all_path.open("w", newline="") as handle:
    writer = csv.DictWriter(
        handle,
        fieldnames=["fusion", "lambda_id", "domain", *metric_names],
    )
    writer.writeheader()
    writer.writerows(sorted(rows, key=lambda row: (row["fusion"], row["lambda_id"], row["domain"])))

grouped = defaultdict(list)
for row in rows:
    grouped[(row["fusion"], row["lambda_id"])].append(row)

summary = []
for (fusion, lambda_id), group in grouped.items():
    if len(group) != 4:
        continue
    averaged = {
        f"mean_{metric}": sum(row[metric] for row in group) / len(group)
        for metric in metric_names
    }
    summary.append({
        "fusion": fusion,
        "lambda_id": lambda_id,
        **averaged,
        "early_retrieval_score": (
            averaged["mean_R@1"] + averaged["mean_R@5"]
        ) / 2.0,
        "official_arithmetic": sum(
            averaged[f"mean_{metric}"]
            for metric in ("R@1", "R@5", "R@10", "R@50")
        ) / 4.0,
    })

summary.sort(key=lambda row: row["official_arithmetic"], reverse=True)
summary_path = output_dir / "summary.csv"
fieldnames = [
    "fusion", "lambda_id", "official_arithmetic", "early_retrieval_score",
    *[f"mean_{metric}" for metric in metric_names],
]
with summary_path.open("w", newline="") as handle:
    writer = csv.DictWriter(handle, fieldnames=fieldnames)
    writer.writeheader()
    writer.writerows(summary)

print(f"Detailed results: {all_path}")
print(f"Four-domain summary: {summary_path}")
if summary:
    best = summary[0]
    print(
        "Best by official four-domain arithmetic mean: "
        f"fusion={best['fusion']:.2f}, lambda_id={best['lambda_id']:.2f}, "
        f"score={best['official_arithmetic']:.4f}, "
        f"early_score={best['early_retrieval_score']:.4f}"
    )
PY
